...
const Hero = () => {
    ...
    useEffect(() => {
        gsap.from('#imgbanner', {
            duration: 1, 
            opacity: 0, // Comienza con una opacidad de 0 (invisible)
            y: -100, // Comienza 100 unidades arriba de su posición final
            ease: 'power2.out'
        });
    
        gsap.to('#imgbanner', {
            delay: 1, // Retraso de 1 segundo después de que la animación de entrada termine
            duration: 1, 
            opacity: 1, // Cambia la opacidad a 1 (visible)
            y: 0, // Mueve el elemento a su posición final
            ease: 'elastic.out(1, 0.5)' // Tipo de suavizado elástico para el efecto de caída
        });
    }, []);
